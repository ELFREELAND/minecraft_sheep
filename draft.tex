...
First, grass in \textit{Minecraft} can spread under fences. Pastures typically have some degree of immigration, causing $\lambda(0)>0$. The effect of immigration was ignored here, since it makes the deterministic model unweildy (see \cite{matis_kiffe} chapter 7). However, introducing immigration would have an important effect on the behavior of the pasture for large $N$, since $N_g=0$ is no longer an inescapeable state.
