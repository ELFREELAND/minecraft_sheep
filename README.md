...
This is important - the steady-state grass growth rate as a function of sheep population density. This is also equal to the rate at which the sheep are eating the grass. We can find the maximum by differentiating with respect to $D$, setting the derivative to 0 and solving for $D$:
...
For the case of the infinitely fast farmer, the rate at which wool is harvested is the same as the rate at which the sheep are eating, which is given above. To account for the farmer being slow, we'll need to think a bit more.

Rather than considering the rate at which sheep eat, we need to consider the rate at which sheep are regrowing their wool. A sheep will only regrow wool upon eating if it doesn't already have any, so we should expect that the rate at which sheep are regrowing wool is the rate at which they are eating times the fraction of sheep that are wool-less. We'll call fraction of sheep that have their wool $W$ \[unitless]. So the rate at which sheep are regrowing their wool is:

$$\frac{dW}{dt}=(1-W)G'$$

Let's say that at time $t=0$, the farmer shears all the sheep, and then at time $t=T$, the farmer comes back and shears them again, gathering $NW=ADW$ blocks of wool. How many sheep grew their wool back in time $T$ \[s]? We can find out by solving the above equation for $W$. We'll separate it into $W$ terms and non-$W$ terms:
...
$$\int_{0}^{W_T}\frac{1}{1-W}dW=\int_{0}^{T}G'dt$$
...
So when the farmer comes back to shear the sheep at $t=T$, they will get $AD(e^{TG'}-1)$ blocks of wool. If the farmer keeps coming back every $T$ seconds, they will be gathering wool at a rate

$$\frac{AD\left(1-e^{-TG'}\right)}{T}$$

We can plug in the expression for $G'$:

$$G' = DR - \frac{D^2R^2}{k}$$

and end up with this:

$$F=\frac{AD\left(1-e^{-T\left(DR - \frac{D^2R^2}{k}\right)}\right)}{T}$$

$$F=-N\left(DR - \frac{D^2R^2}{k}\right)$$

Where F \[m<sup>2</sup>s<sup>-1</sup>] is the average rate at which the farmer gathers wool from a pasture with sheep density $D$ if they come back to shear the sheep every $T$ seconds.

Our farm now has two variables - the sheep density $D$, and the shearing time $T$. We'll need to optimize with respect to both of these variables. We can do this by taking the derivative with respect to $D$ and $T$ and setting them to 0, as before:

$$\frac{dF}{dD}=\frac{d}{dD}\left[\frac{AD\left(1-e^{-T\left(DR - \frac{D^2R^2}{k}\right)}\right)}{T}\right]$$

$$=\frac{A}{T}-\left(e^{-T\left(DR-\frac{D^2R^2}{k}\right)}\left(\frac{A}{T}-ADR+\frac{2AD^2R^2}{k}\right)\right)$$
