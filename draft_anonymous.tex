...
As before, this system can be solved numerically. The initial values of $q_{x,0}$ were set using the values of $p_x(t=10000)$ from the grass population model. Figure \ref{fig:fslow_stochastic} shows $F_{slow}$ as calculated from the results of this model. Similar to what we saw in Figure \ref{fig:stochastic_Ng}, the stochastic model predicts significantly lower $F_{slow}$ than the deterministic model.
